 ...
 * @api public
 */
function CustomSessionStore(options) {
  if (!options.key) {
 ...
 * @api protected
 */
CustomSessionStore.prototype.store = function (req, callback) {
  if (!req.session) {
    return callback(
      new Error(
  ...
  var key = this._key
  var state = JSON.stringify({ state: req.session.state, uid: uid(24) })
  if (!req.session[key]) {
    req.session[key] = {}
  }
 ...
 * @api protected
 */
CustomSessionStore.prototype.verify = function (req, providedState, callback) {
  if (!req.session) {
    return callback(
      new Error(
        'OAuth 2.0 authentication requires session support when using state. Did you forget to use express-session middleware?',
